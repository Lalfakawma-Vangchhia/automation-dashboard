...
async def get_auth_token(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        # Retrieve the stored access token so the frontend can use it (e.g., Google Picker)
        access_token = None
        if os.path.exists(TOKEN_FILE):
            try:
                token_creds = Credentials.from_authorized_user_file(TOKEN_FILE, SCOPES)
                access_token = token_creds.token
            except Exception as token_err:
                logger.warning(f"Unable to load access token from {TOKEN_FILE}: {token_err}")
